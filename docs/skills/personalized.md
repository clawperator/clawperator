...
This page covers personal AgentSkills and OpenClaw workspace skills that call
Clawperator runtime skills. It does not change the runtime skills registry
contract documented in [Overview](overview.md), and it does not add a
Clawperator memory or preference store.
...
but the wrapper remains personalized if it maps `<user_room_alias>` to one
user's actual `<zone_label>`.
...
2. If the runtime skill is a clear match but needs local defaults, create a
   personal AgentSkills wrapper rather than changing the runtime registry.
...
| --- | --- | --- |
| Runtime skill | Installed skills registry, normally `~/.clawperator/skills/skills/skills-registry.json` | Generic execution package when its inputs and verification do not depend on one user's private setup. |
| Personal AgentSkills wrapper | User-owned agent skill directory such as `~/.agents/skills/` | Local or user-scoped wrapper that may encode private defaults outside public artifacts. |
| OpenClaw workspace skill | `~/.openclaw/workspace/skills/` | Workspace-scoped agent skill for one OpenClaw workspace. |
| Bundled authoring skill | `~/.clawperator/bundled-skills/` plus host discovery entries | First-party authoring workflow, not a runtime app automation skill. |
