#!/usr/bin/env bash
# Opt-in real-device integration check: compile known artifact, execute, assert canonical envelope.
...
EXEC_OUT="$("${CLI[@]}" execute --device "$DEVICE_ID" --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" --execution /tmp/clawperator-integration-exec.json --json 2>&1)" || true
