...
- Manual physical-device smoke using a real user-originated Back press still did not produce a `press_key` record. Session `manual-back-verify-20260319-093607` on a physical Samsung device recorded `11` lines total (`1` header + `10` events) with event mix `window_change=7`, `click=4`, `press_key=0`. The flow used real interaction in Play Store: dismiss popup, open account menu, then press the hardware/system Back control once. Result: no `{"type":"press_key","key":"back",...}` line appeared in the NDJSON, so the current key-filtering capability declaration is insufficient for end-to-end Back capture on this device/runtime path.
- Proof-of-concept scope decision: hardware Back capture moves to a dedicated follow-up task. Phase 1 proves that recording and replay can work for flows that use click / scroll / text_change and avoid hardware Back, which is still a valid proof-of-concept win. Do not assume Back-based flows are recordable until the missing `press_key` path is solved.
