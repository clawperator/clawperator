...
3. Build before test. Run `npm --prefix apps/node run build` before every
   `npm --prefix apps/node run test` invocation. Never run them in parallel.
4. Add tests in the same phase that introduces behavior. Do not defer tests to a
   later cleanup phase.
5. Preserve every supported raw event type in the export path. Do not silently
   drop `scroll`, `text_change`, or `press_key`.
6. Do not edit `sites/docs/.build/` or `sites/docs/site/` directly. Author docs
   in `docs/`, then regenerate.
7. Use the branch-local build from `apps/node/` for all validation, not the
   globally installed `clawperator` binary.
8. Keep the recording-context copy verbatim. Do not rewrite, normalize, or
   summarize the exported JSON during scaffold copy.
9. Leave `skill.json.artifacts` unchanged for this task. The recording context
   is a reference file, not a recipe artifact.
10. If a phase uncovers a plan deviation, update `plan.md` first, then continue.
    Do not silently drift.
11. One commit per phase. Do not batch unrelated changes.
12. Never shorten `Clawperator` to `Claw` in code, docs, comments, or commit
    messages.
13. For the live validation phase, prefer the debug Operator APK and pass
    `--operator-package com.clawperator.operator.dev`.
14. Create `tasks/api/recording-export-agent-context/findings.md` at the start
...
4. Create `apps/node/src/test/unit/recordingExport.test.ts` and define the full
   required-case matrix before implementing runtime logic.
5. Required cases:
   ...
   - package transitions are emitted only when adjacent package-bearing events
     change package
   - missing snapshot stays `present: false` with `xml: null`
   - malformed header fails
   - malformed event fails
   - unsupported schema version fails
6. If contract changes are needed, update `plan.md` before proceeding.

### Acceptance Criteria

- Export types exist in `recordingEventTypes.ts`
- `recordingExport.test.ts` names all required cases explicitly
...
```text
feat(node): define recording export contract and tests
```
...
1. Implement `exportRecordingFile(inputFile, outputFile?)` and any pure helper
   functions in a new `exportRecording.ts`.
2. Reuse the same NDJSON validation rules as the current parser. Do not fork a
   second inconsistent validation contract.
3. Preserve all validated raw events in the exported `events` array.
4. Compute only the deterministic derived fields from `plan.md`:
   - sorted event order
   - `deltaMsSincePrevious`
   - counts
   - package transitions
5. Add `cmdRecordExport()` to `apps/node/src/cli/commands/record.ts`.
6. Wire `recording export` and `record export` help text and usage into
   `apps/node/src/cli/registry.ts`.
7. Keep `recording parse` unchanged.
8. Add CLI tests for:
...
4. Update scaffolded `SKILL.md` starter text so it explains that the recording
   context is reference evidence for an external agent to use while authoring
   the skill.
5. Do not change the scaffolded `run.js` logic beyond wording that points users
   toward the recording context for manual / agent refinement.
6. Add skills tests for:
...
4. Create `tasks/api/recording-export-agent-context/findings.md` at the start
   ...
   - `## Anomalies`
   - `## Open Questions`
5. If a debug Operator device is available:
   - run `recording start`
...
6. If a device is not available, note the host-state limitation in
   `findings.md` and treat the unit test suite as the primary gate.
7. Use `.agents/skills/docs-build/SKILL.md` or the repo-standard build commands
