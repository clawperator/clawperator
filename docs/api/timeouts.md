...
| `wait_for_node` via builder | `params.timeoutMs` | optional in raw schema, builder sets it when `--timeout` is passed |
...
This limit belongs to the best-effort execution mode documented elsewhere in the codebase. It is not the same as the standard direct execution timeout range. For normal direct executions, agents should still keep top-level `timeoutMs` inside `1000..120000`.
...
- default `30000` leaves room for dispatch and envelope handling
