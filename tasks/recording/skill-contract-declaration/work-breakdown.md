...
6. Commit the exact v1 Solax-friendly shape so the video draft and downstream
   packs are not guessing.
...
- The pack commits to a concrete v1 declaration shape for the Solax proving
  case, including:
  - `inputs: { "percent": "integer[0,100]" }`
  - `goal: { "kind": "set_discharge_limit" }`
  - `verification: { "kind": "node_text_matches", "matcher": "Discharge to {percent}%" }`
