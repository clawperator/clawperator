...
To capture a visible panel with the current generic transport, use separate raw
...
the panel remains visible, then inspect the caller-selected output file. Do not
use a successful draw acknowledgement as proof of screenshot inclusion.
...
The actions are validated before Android dispatch. Invalid input returns
`EXECUTION_VALIDATION_FAILED`; it cannot change an existing panel. Runtime
failures are described in [Errors](errors.md#on-screen-log-panel-failures).
