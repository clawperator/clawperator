...
if [ -n "$SMOKE_SUMMARY" ]; then
  EXEC_JSON="$("${CLI[@]}" execute --device "$DEVICE_ID" --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" --execution "$SMOKE_JSON" --json 2>&1)" || true
  ...
  EXEC_OUT="$("${CLI[@]}" execute --device "$DEVICE_ID" --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" --execution "$SMOKE_JSON" --output pretty)" || true
  echo "$EXEC_OUT"
fi
...
echo "=== snapshot ==="
if [ -n "$SMOKE_SUMMARY" ]; then
  OBS_JSON="$("${CLI[@]}" snapshot --device "$DEVICE_ID" --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" --json 2>&1)" || true
  ...
  OBS_OUT="$("${CLI[@]}" snapshot --device "$DEVICE_ID" --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" --output pretty)" || true
  echo "$OBS_OUT"
fi
...
if ! "${CLI[@]}" read \
  --device "$DEVICE_ID" \
  --operator-package "$CLAWPERATOR_RECEIVER_PACKAGE" \
