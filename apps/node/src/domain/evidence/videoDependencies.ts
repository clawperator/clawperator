...
export async function inspectVideoDependencies(runner: ProcessRunner): Promise<VideoDependencyIssue[]> {
  const probes = [
    ...
    { dependency: "ffmpeg", args: ["-hide_banner", "-encoders"], requirement: "FFmpeg with the libx264 encoder", supports: (text: string) => /\blibx264\b/.test(text) },
  ...
  const results = await Promise.all(probes.map(async probe => {
    ...
    try {
      ...
      else if (!probe.supports(result.stdout + result.stderr)) reason = "unsupported";
    } catch (error) {
