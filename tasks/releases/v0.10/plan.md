...
R1-R7 and R10-R13 implementations are merged. R11 landed in `28b8b1fa` (PR #281), R12 in `460e654c` (PR #280), and R13 in `0c4ed5ce` (PR #282). The integrated local API 35 matrix passes with the Internet-readiness and Operator-setup follow-ups through `306b38d`; R12's pack is retired. Two task packs remain active: result transport's retained causal/release follow-up and the two evidence-capture phases. The manual supported-image release CI gate remains required.
