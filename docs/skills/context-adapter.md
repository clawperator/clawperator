...
nodes, including hidden nodes, and a visible enabled clickable node or ancestor.
...
`read-value --label` associates a label and value; query `text` and returned
`label` are different contracts. Query results use numeric bounds and are
