...
- handle `error.code` values such as `SKILL_OUTPUT_ASSERTION_FAILED`, `SKILL_EXECUTION_FAILED`, <code>SKILL_EXECUTION_<wbr>TIMEOUT</code>, and `SKILL_RESULT_PARSE_FAILED` through the nested `error` object, not as the top-level response object
...
### <code>SKILL_EXECUTION_<wbr>TIMEOUT</code>
