...
An orchestrated skill is not just replaying taps. It's a collaboration between the brain and the hand.
...
So after this recording step, what we're going to do is inspect the captured output, scaffold the skill, and then separate the simple replay baseline from the orchestrated version. The replay version is useful because it's our foundation. It tells us, yes, there is a valid deterministic route through this UI. But the orchestrated version is what lets the agent really own the outcome.
