...
def test_write_preflight_failure_run_persists_full_doctor_report_in_full_repo(monkeypatch, tmp_path):
    ...
    preflight_details = {
        "doctor_report": {
            "deviceId": "emulator-5554",
            "operatorPackage": "com.clawperator.operator.dev",
            "nextActions": ["Run `clawperator doctor --output json`"],
        },
        "doctor_failure": {
            "code": "VERSION_INCOMPATIBLE",
            "summary": "CLI and APK versions are incompatible",
    ...
    assert result["preflight"]["doctor_report"]["nextActions"] == ["Run `clawperator doctor --output json`"]
...
def _valid_discovery_artifact_json(
    *,
    ...
    runtime_command: str = "clawperator skills for-app com.android.settings --output json",
    authoring_command: str = "clawperator bundled-skills list --output json",
...
def test_attach_skill_score_requires_pack_a_route_evidence(monkeypatch, tmp_path):
    ...
    run_dir.mkdir()
    (run_dir / "transcript.txt").write_text(
        '{"type":"item.completed","item":{"type":"command_execution","command":"clawperator bundled-skills list --output json"}}\n'
    ...
    assert updated["skill_score"]["bundled_skills_list_seen"] is True
...
def test_attach_skill_score_accepts_pack_a_route_evidence(monkeypatch, tmp_path):
    ...
    (run_dir / "transcript.txt").write_text(
        (
            '{"type":"item.completed","item":{"type":"command_execution","command":"node apps/node/dist/cli/index.js skills for-app com.android.settings --output json"}}\n'
            ...
            + _valid_discovery_artifact_json(
                runtime_command="node apps/node/dist/cli/index.js skills for-app com.android.settings --output json",
...
def test_attach_skill_score_accepts_equivalent_registry_launchers(monkeypatch, tmp_path):
    ...
    (run_dir / "transcript.txt").write_text(
        (
            ...
            "Using clawperator-skill-author-by-agent-discovery for bounded discovery\n"
            + _valid_discovery_artifact_json(
                runtime_command="clawperator skills for-app com.android.settings --output json",
                authoring_command="clawperator bundled-skills list --output json",
            )
            + "CLAWPERATOR_SKILL_START\n"
            + _skill_payload_json()
            + "\n"
...
def test_attach_skill_score_requires_explicit_front_door_signal(monkeypatch, tmp_path):
    ...
    (run_dir / "transcript.txt").write_text(
        (
            '{"type":"item.completed","item":{"type":"command_execution","command":"clawperator skills for-app com.android.settings --output json"}}\n'
            '{"type":"item.completed","item":{"type":"command_execution","command":"clawperator bundled-skills list --output json"}}\n'
            + _valid_discovery_artifact_json()
            + "CLAWPERATOR_SKILL_START\n"
            + _skill_payload_json()
...
def test_attach_skill_score_rejects_copied_registry_provenance(monkeypatch, tmp_path):
    ...
    (run_dir / "transcript.txt").write_text(
        (
            ...
            '{"type":"item.completed","item":{"type":"command_execution","command":"clawperator bundled-skills list --output json"}}\n'
            + _valid_discovery_artifact_json(
                runtime_command='clawperator skills search --keyword "Netflix" --output json',
                authoring_command="clawperator bundled-skills list --output json",
            )
            + "CLAWPERATOR_SKILL_START\n"
            + _skill_payload_json()
...
def test_attach_skill_score_requires_runtime_discovery_before_authoring(monkeypatch, tmp_path):
    ...
    assert updated["skill_score"]["runtime_skill_discovery_before_authoring"] is False
    assert updated["skill_score"]["route_requirements_met"] is False
    assert "runtime-skill discovery must appear before `clawperator bundled-skills list --output json`" in updated["skill_score"]["route_requirement_errors"]
    assert updated["outcome"]["status"] == "fail"
