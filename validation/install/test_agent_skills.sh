...
assert_contains "$BRIDGE_EXISTING_PATH" 'Confirm the local guide lists `clawperator-agent-orientation`, `clawperator-upgrade`, `skill-author-by-agent-discovery`, and `skill-author-by-recording` before starting the discovery-to-proving route.' "bridge-existing file"
