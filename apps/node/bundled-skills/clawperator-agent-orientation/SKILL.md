...
| --- | --- |
| You are trying to solve a user-facing app workflow and want an installed runtime skill first | `clawperator skills for-app <package_id> --output json` or `clawperator skills search --keyword <text> --output json` |
...
| --- | --- |
| runtime skills with known package id | `clawperator skills for-app <package_id> --output json` |
| runtime skills with only user-language terms | `clawperator skills search --keyword <text> --output json` |
| zero-results authoring route | `clawperator bundled-skills list --output json` |
| proving workflow | `clawperator bundled-skills list --output json` |
| MCP | `clawperator mcp serve` |
| raw CLI / direct actions | `clawperator snapshot --output json --device <device_serial>` |

Rules:

- do not start runtime-skill discovery with `clawperator skills list --output json` unless the real task is inventory
- do not inspect `clawperator bundled-skills list --output json` before runtime-skill discovery unless the route is already known to be authoring
...
- "Your next step is `clawperator skills for-app <package_id> --output json`."
