...
- verify the resulting install with `clawperator doctor --output json`
- when multiple devices are connected, verify each connected device with
  `clawperator doctor --output json --device <device_id>` before claiming the whole
  host is ready
...
- do not invent a second upgrade-health checker beyond `clawperator doctor --output json`
...
- keep `clawperator doctor --output json` as the readiness check after the sequence
- if the install result has `deviceSelectionRequired: true`, collect the
  ...
  `clawperator doctor --output json --device <device_id>` for each connected device
- if `clawperator doctor --output json` returns `MULTIPLE_DEVICES_DEVICE_ID_REQUIRED`,
...
- if the goal is runtime-skill discovery, suggest
  `clawperator skills for-app <package_id> --output json` or
  `clawperator skills search --keyword <text> --output json`
