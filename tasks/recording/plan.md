...
Also complete as cleanup:

- `tasks/recording/demo/` deleted after its durable content graduated
- `tasks/recording/brain-hand-contract/` deleted after its durable rationale
  was either implemented or no longer needed as a live task reference
...
- `tasks/recording/demo/` is deleted
- `brain-hand-contract/` is deleted once the contract docs land
- this top-level plan is deleted when no active recording workstreams remain
...
Originally, that work succeeded enough to expose a broader problem:

- recordings are evidence, not executable skills
- non-trivial skills need truthfulness and terminal verification
- the brain/hand split was not real at the skill boundary because skills
  returned opaque stdout instead of structured results
...
| --- | --- | --- | --- |
| 0 | `brain-hand-contract/` | retired | problem definition and architectural framing formerly lived here; durable shipped behavior now lives in docs and code |
...
| 2b | `agent-driven-skills/` | complete | define the runtime agent shape for orchestrated skills and prove it on the Solax `-orchestrated` skill |
| 2c | `agent-driven-skills-closeout/` | complete | close macro-review gaps, finish reliability validation, and make the W2b branches honestly PR-ready |
| 3 | `skill-contract-declaration/` | complete | declare inputs, goal, and verification in the agent-driven orchestrated `skill.json` |
| 4 | `compare/` | complete | compare recording baseline against emitted `SkillResult`, including agent-driven runs that may take a different path |
| 4c | `compare-closeout/` | complete | harden compare behavior, close follow-up findings, and make compare fail closed |
| 5 | `graduate-demo-findings/` | complete | graduate durable recording and authoring facts into docs and delete temporary demo notes |
| 6 | `skill-author-by-recording/` | next | package the proven recording-to-orchestrated workflow into a repo-local agent skill |
...
1. `skill-checkpoints/`
2. `skill-result-contract/`
3. `agent-driven-skills/`
4. `agent-driven-skills-closeout/`
5. `skill-contract-declaration/`
6. `compare/`
7. `graduate-demo-findings/`

Remaining sequence:

1. Start `skill-author-by-recording/`

`graduate-demo-findings/` no longer blocks anything on this branch. W6 is now
the remaining recording-program workstream in this repo subtree.
