...
| 3 | `validation/install/test_agent_skills.sh`, `validation/install/test_main.sh`, and `validation/install/test_install.sh` when touched | installer setup, guide text, and local summary surfaces mention `clawperator-upgrade` correctly and idempotently | `bash -n`, stdout inspection, or one-off local runs |
| 4 | docs build plus installer harness reruns | authored docs and machine-facing guidance match the shipped installer/help behavior | docs edits without build, or build without rereading the content against code |
...
- `sites/landing/public/llms-full.txt` if the final surface inventory or
  machine-facing guidance changes there
...
5. If landing machine-facing guidance lists the installed first-party
   `agent-skills`, update `sites/landing/public/llms-full.txt` in the same
   phase.
