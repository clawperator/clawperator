...
| Serve API | `clawperator serve` then HTTP requests | One long-running Node process | Repeated agent loops and throughput-sensitive callers |
...
For repeated agent loops, `serve` is the public persistent API to prefer.
...
The harness first runs `clawperator close --app <package>`, then
`clawperator open --app <package>`, so each measured app starts from its normal
