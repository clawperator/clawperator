...
| Rule | Meaning |
| --- | --- |
| Canonical action names only | Stored payloads should use canonical types such as `open_uri`, `wait_for_node`, and `take_screenshot`. Input aliases are normalized before validation except for the canonical-only on-screen log actions. |
...
The two on-screen log actions are exceptions to the generic input-alias rules:

- `set_on_screen_log` and `clear_on_screen_log` must use those exact lower-case names, without aliases, case changes, or surrounding whitespace.
