...
assert_json_field_equals "$SUCCESS_MCP_CONFIG_PATH" "notes.1" "Regenerate it with clawperator host materialize-artifacts if the clawperator binary path or adb path changes." "main-success mcp notes.1"
...
assert_contains "$SUCCESS_CLI_LOG" "host materialize-artifacts --output json --cli-version 1.2.3 --last-device-serial serial-solo" "main-success cli log"
...
assert_contains "$FAIL_CLI_LOG" "host materialize-artifacts --output json --cli-version 1.2.3 --last-device-serial serial-solo" "main-fail cli log"
