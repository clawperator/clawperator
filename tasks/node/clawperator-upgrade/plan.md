...
| `sites/landing/public/install.sh` | Installer-written local guide, four-skill completeness gate, and install references to `clawperator-upgrade` | PR-2 / Phase 3 |
| `validation/install/test_agent_skills.sh`, `validation/install/test_main.sh`, `apps/node/src/test/integration/installScript.test.ts` | Installer-facing regression coverage for the fourth skill and new guide text, while keeping the rest of `install.sh` behavior green | PR-2 / Phase 3 |
...
| `sites/landing/public/llms-full.txt` | Generated machine-facing artifact refreshed by `./scripts/docs_build.sh` when the authored docs change | PR-2 / Phase 4 validation |
...
| Installer validation harnesses | `validation/install/README.md`, `validation/install/test_agent_skills.sh`, `validation/install/test_main.sh`, `validation/install/test_install.sh`, `apps/node/src/test/integration/installScript.test.ts` |
...
- `sites/landing/public/install.sh` currently decides whether the installed
  front doors are "complete" using explicit per-skill booleans. That branch
  must move from a three-skill gate to a four-skill gate so the installer does
  not incorrectly report a healthy first-run route while `clawperator-upgrade`
  is missing.
...
- `sites/landing/public/llms-full.txt` is generated by the docs build. Do not
  hand-edit it as an authored surface in this task pack.
...
- `install.sh` still writes local guidance that assumes only three first-party
  `agent-skills`.
- `install.sh` treats orientation + discovery + recording as a sufficient
  "complete" set and therefore omits upgrade guidance even when the new skill
  is missing.
...
- Public docs mention the old first-party set or fail to introduce the upgrade
  route at all.
- `sites/landing/public/llms-full.txt` is edited directly instead of being
  regenerated through `./scripts/docs_build.sh`.
