...
agent that reasons turn by turn against a declared contract.

See `tasks/recording/refactor-plan.md` for the architectural framing.
...
- `skill.json` `agent` block (cli, optional cliPath, optional
  timeoutMs, optional minVersion)
...
- a new `clawperator doctor` check for agent CLI availability
...
- input validation (`skill.json` declared inputs enforced by
  `runSkill` before the agent is spawned)
...
| `apps/node/src/domain/doctor/checks/` | Clawperator repo | new agent CLI availability check |
...
- `runSkill` validates declared inputs against the `skill.json`
  inputs schema **before** spawning the agent. The agent never sees
  invalid inputs.
...
- Exact primitive allowlist the runtime agent is told it can use.
  Recommend v1: `clawperator exec`, `clawperator snapshot`,
  `clawperator checkpoint`, `clawperator skill-result emit` (or
  equivalent). Must be verified against `apps/node/src/cli/registry.ts`
  before landing.
...
- `runSkill` changes to detect agent-driven skills, validate inputs,
...
- a `clawperator doctor` check for the configured agent CLI on PATH
