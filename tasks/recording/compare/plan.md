...
- Compare must support sanitized retained baseline exports whose sensitive
  values have been templated to placeholders such as `<device_serial>`,
  `<person_name>`, `<place_name>`, and `<account_email>`. Baseline
  normalization should rely on stable structural
  ...
  compare outcome.
- The canonical retained baseline for an authored skill lives under a
  reference-style path such as `references/compare-baseline.export.json`.
  It is authoring evidence for compare and maintenance, not a runtime artifact,
  and must not be listed under `skill.json.artifacts`.
...
- clear support for compare reading the full saved
  `clawperator skills run --json` wrapper and extracting `skillResult`
