...
2. Extract the release date from the `RELEASE_DATE:` line.

3. Derive the version by stripping the `v` prefix from the end tag. For example, `v0.5.1` becomes `0.5.1`.

4. Process every commit block using only the `CLASSIFICATION:` line. Never re-derive keep/drop from the file list.

`drop:no-src` and `drop:infra` mean skip the commit, but log the SHA and classification in `tasks/release-notes/findings.md` as part of the classification summary table. Never skip silently.
...
5. Synthesize `keep` commits into bullets.
...
Breaking changes require explicit evidence in the `SUBJECT` or `BODY`, or a deleted `[src]` file that represents a user-facing capability with no replacement in the same commit. Use `**Breaking:** **Removed:**` when a user-facing capability was deleted without replacement. Use `**Breaking:** **Changed:**` when the deletion is part of a rename or replacement. If the deleted source file was purely internal and there is no user-facing change, record that in `tasks/release-notes/findings.md` with the SHA and reason, then do not invent a changelog bullet.

Merged commits:

Commits that share the same `PR:` number must be merged into one bullet group. Outside the same PR, merge only when the commits share adjacent `FILES` entries in the same module or explicitly cross-reference each other in the `BODY`. Record merged SHAs in `tasks/release-notes/findings.md`.
...
Every `keep` commit must produce at least one bullet or be explicitly escalated in `tasks/release-notes/findings.md`. No silent omissions. Before writing the `CHANGELOG.md` block, record every `keep` commit SHA and the bullet or bullets it produces in `tasks/release-notes/findings.md`.

Within each surface section, order bullets as Added, then Changed, then Fixed, then Removed.

6. Write a one-or-two sentence summary.
...
7. Assemble the changelog block in this format.
...
8. Apply the upsert rule to `CHANGELOG.md` exactly as specified in `tasks/release-notes/plan.md` under `CHANGELOG Insertion Rule (Upsert)`. Do not re-derive the logic. Follow the five-state table verbatim.

9. Verify that no duplicate version headers exist and that entries remain in descending chronological order.
