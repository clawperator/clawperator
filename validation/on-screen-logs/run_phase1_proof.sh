...
usage() {
    cat <<'EOF'
Usage: validation/on-screen-logs/run_phase1_proof.sh --device <device_serial> --output-dir <absolute_path>

Runs the debug-only Phase 1 overlay proof. The selected device must already have the branch's
...
remote_video="/sdcard/on-screen-log-phase1.mp4"
...
wait "$recording_pid"
"${adb_cmd[@]}" pull "$remote_video" "$output_dir/on-screen-log-phase1.mp4" >/dev/null
"${adb_cmd[@]}" shell rm -f "$remote_video"
...
echo "Phase 1 captures written to $output_dir"
