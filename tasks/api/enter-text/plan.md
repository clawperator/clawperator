...
| API 33 accessibility IME capability flag | `AccessibilityServiceInfo.FLAG_INPUT_METHOD_EDITOR` - must be added to `android:accessibilityFlags` in `apps/android/shared/data/resources/src/main/res/xml/accessibility_service_config.xml`. Read the current attribute value before Phase 3 begins and combine the new flag rather than overwriting existing flags. |
