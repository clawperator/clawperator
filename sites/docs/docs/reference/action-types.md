...
**Note on `clickAfter`:** The click only fires if the loop terminates with `TARGET_FOUND`. If termination is `EDGE_REACHED` (even if target is visible), no click occurs.
